...
public section.
  ...
  class-methods DISPLAY_SIMPLE
    importing
      value(IV_PROGID) type SY-CPROG default SY-CPROG
      value(IV_ALV_KEY) type CHAR10 default 'BALV'
      value(IV_FCAT_KEY) type CHAR10 default 'DEFAULT'
      !IT_SLIS_FCAT type SLIS_T_FIELDCAT_ALV optional
      !IS_SLIS_LAYOUT type SLIS_LAYOUT_ALV optional
      value(IV_CALLBACK_PROGID) type SY-CPROG default SY-CPROG
      !IT_SLIS_SORT type SLIS_T_SORTINFO_ALV optional
      !IV_USE_CONFIG type CHAR01 default 'X'
      !IV_USE_COMMAND type CHAR01 default ' '
    changing
      !CT_DATA type TABLE .
  class-methods DISPLAY_SIMPLE_NEW
    importing
      !IT_FIELDCAT type LVC_T_FCAT optional
      !IT_TOOLBAR_EXCLUDING type UI_FUNCTIONS optional
    ...
    changing
      !CT_DATA type TABLE .
  class-methods DISPLAY_SIMPLE_POPUP
    importing
      value(IV_PROGID) type SY-CPROG default SY-CPROG
      value(IV_ALV_KEY) type CHAR10 default 'BALV'
      value(IV_FCAT_KEY) type CHAR10 default 'DEFAULT'
  ...
  methods INSERT_TOOLBAR_BUTTON
    importing
      ...
      value(IV_CHECKED) type CHAR1 optional
      !IV_INDEX type I .
  methods IS_CHANGE_MODE
    returning
      value(RV_RETURN) type CHAR1 .
  methods IS_CREATE_MODE
    returning
      value(RV_RETURN) type CHAR1 .
  methods IS_DISPLAY_MODE
    returning
      value(RV_RETURN) type CHAR1 .
  methods IS_UPLOAD_MODE
    returning
      value(RV_RETURN) type CHAR1 .
  methods REFRESH
    importing
      !IV_MODE type CHAR4 optional .
  class-methods ROLLBACK_FAIL .
  methods SET_ALL_FIELD_CHANGEABLE .
  methods SET_ALL_FIELD_UNCHANGEABLE .
  methods SET_ALV_MODE
    importing
      value(IV_MODE) type CHAR04 .
  methods SET_CELL_CHANGEABLE
    importing
      value(IV_FIELDNAME) type FIELDNAME
      value(IV_ROWID) type I .
  methods SET_CELL_COLOR
    importing
      value(IV_FIELDNAME) type FIELDNAME
      value(IV_ROWID) type I
      value(IV_COLOR) type CHAR03 .
  methods SET_CELL_UNCHANGEABLE
    importing
      value(IV_FIELDNAME) type FIELDNAME
      value(IV_ROWID) type I .
  ...
  methods SET_ALV_CREATABLE
    importing
      !IV_FLAG type FLAG .
  methods SET_ALV_UPDATABLE
    importing
      !IV_FLAG type FLAG .
  methods SET_ALV_DELETABLE
    importing
      !IV_FLAG type FLAG .
  methods SET_ALV_UPLOADABLE
    importing
      !IV_FLAG type FLAG .
  ...
  PROTECTED SECTION.
    ...
    DATA ms_celltab TYPE lvc_s_celltab .
    DATA ms_coltab TYPE lvc_s_coltab .
...
CLASS ZCL_BRD_ALV IMPLEMENTATION.
...
*        lo_parent ?= NEW cl_gui_custom_container( container_name = iv_parent ).
  ...
  METHOD delete_selected_rows.
    ...
    ASSIGN mt_outtab->* TO <fs_tab>.
    LOOP AT lt_rows INTO ls_rows WHERE rowtype EQ space.
      DELETE <fs_tab> INDEX ls_rows-index.
    ENDLOOP.
  ...
  METHOD display.
    ...
    " 6. celltab설정
*    set_celltab( ).
    IF ms_layout-stylefname IS NOT INITIAL.
      LOOP AT mt_celltab INTO ms_celltab.
        READ TABLE <lt_outtab> ASSIGNING <ls_data_line> INDEX ms_celltab-rownum.
        IF <ls_data_line> IS ASSIGNED.
          ASSIGN COMPONENT ms_layout-stylefname OF STRUCTURE <ls_data_line> TO <lt_celltab>.
          IF <lt_celltab> IS ASSIGNED.
            <lt_celltab> = ms_celltab-celltab.
            UNASSIGN <lt_celltab>.
            CLEAR ms_celltab-celltab.
          ENDIF.
          UNASSIGN <ls_data_line>.
          CLEAR ms_celltab-celltab.
        ENDIF.
        CLEAR ms_celltab-celltab.
      ENDLOOP.
    ...
    " 7. coltab설정
*    set_coltab( ).
    IF ms_layout-ctab_fname IS NOT INITIAL.
      LOOP AT mt_coltab INTO ms_coltab.
        READ TABLE <lt_outtab> ASSIGNING <ls_data_line> INDEX ms_coltab-rownum.
        IF <ls_data_line> IS ASSIGNED.
          ASSIGN COMPONENT ms_layout-ctab_fname OF STRUCTURE <ls_data_line> TO <lt_coltab>.
          IF <lt_coltab> IS ASSIGNED.
            <lt_coltab> = ms_coltab-coltab.
            UNASSIGN <lt_coltab>.
            CLEAR ms_coltab-coltab.
          ENDIF.
          UNASSIGN <ls_data_line>.
          CLEAR ms_coltab-coltab.
        ENDIF.
  ...
  METHOD display_simple.
    DATA : lt_slis_fcat             TYPE slis_t_fieldcat_alv,
           ...
           lv_callback_user_command TYPE  slis_formname.
    ...
    IF iv_use_config EQ abap_true.
      CALL METHOD zcl_brd_alv=>get_slis_config
        EXPORTING
          iv_progid      = iv_progid
          iv_alv_key     = iv_alv_key
          iv_fcat_key    = iv_fcat_key
        IMPORTING
          et_slis_fcat   = lt_slis_fcat
          es_slis_layout = ls_slis_layout
          et_slis_sort   = lt_slis_sort.
    ELSE.
      ls_slis_layout = is_slis_layout.
      lt_slis_fcat = it_slis_fcat.
      lt_slis_sort = it_slis_sort.
    ENDIF.


    IF lt_slis_fcat IS INITIAL.

      table_descr ?= cl_abap_typedescr=>describe_by_data( ct_data ).
      ...
      LOOP AT columns ASSIGNING <column>.
        IF <column>-type_kind EQ 'h'.
          CONTINUE.
        ENDIF.
        ls_slis_fcat-fieldname = <column>-name.
        ls_slis_fcat-seltext_l  = <column>-name.
        ls_slis_fcat-seltext_m  = <column>-name.
        ls_slis_fcat-seltext_s  = <column>-name.
        ls_slis_fcat-intlen = <column>-length.
        ls_slis_fcat-decimals_out = <column>-decimals.
        APPEND ls_slis_fcat TO lt_slis_fcat.
      ENDLOOP.
    ENDIF.

    IF iv_use_command EQ abap_true.
      lv_callback_user_command = 'USER_COMMAND'.
    ENDIF.

    CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
      EXPORTING
        i_callback_program      = iv_callback_progid
        i_callback_user_command = lv_callback_user_command
*       i_grid_title            = ls_slis_layout-grid_title
  ...
  METHOD display_simple_new.
    CALL FUNCTION 'ZCMZ_ALV_DISPLAY'
      EXPORTING
        iv_progid           = iv_progid
        iv_alv_key          = iv_alv_key
        iv_fcat_key         = iv_fcat_key
      ...
      TABLES
        ...
        t_sortcat           = it_sortcat
        t_drop_down         = it_drop_down
        t_toolbar_excluding = it_toolbar_excluding.

  ENDMETHOD.


  METHOD display_simple_popup.
    DATA : lv_title TYPE ztbrd00010-grid_title,
           lv_zebra.
    DATA : lv_exit.
    ...
    SELECT SINGLE grid_title zebra INTO ( lv_title, lv_zebra )
    FROM ztbrd00010
    WHERE progid EQ iv_progid
      AND alv_key EQ iv_alv_key.


    zcl_brd_alv=>get_fieldcat_config( EXPORTING iv_progid = iv_progid
                                                iv_alv_key = iv_alv_key
                                                iv_fcat_key = iv_fcat_key
...
*    ENDIF.
  ...
  METHOD handle_toolbar.
    ...
    IF ms_header_config-zhelp EQ abap_true.
      _set_toolbar ' ' 'BAHP' icon_system_extended_help 'Help Guide' '' ' '.
      _set_toolbar '3' '' '' '' '' ' '.
    ENDIF.
    CASE mv_mode.
      WHEN mc_upload_mode.
        IF ms_header_config-zexcel EQ abap_true.
          _set_toolbar ' ' 'SAVE' icon_system_save 'Save' 'Save' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
          _set_toolbar ' ' 'DELE' icon_delete 'Delete' 'Delete' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
          _set_toolbar ' ' 'CANC' icon_cancel 'Cancel' 'Cancel' ' '.
        ENDIF.
      WHEN mc_display_mode.
        IF ms_header_config-zrefresh EQ abap_true.
          _set_toolbar ' ' 'RFSH' icon_refresh 'Refresh' 'Refresh' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
        ENDIF.
        IF ms_header_config-zcreate EQ abap_true.
          _set_toolbar ' ' 'CREA' icon_create 'Create' 'Create' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
        ENDIF.
        IF ms_header_config-zupdate EQ abap_true.
          _set_toolbar ' ' 'CHAN' icon_change 'Change' 'Change' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
        ENDIF.
        ...
        ENDIF.
        IF ms_header_config-zexcel EQ abap_true.
          _set_toolbar ' ' 'UPLD' icon_xls 'Excel Upload' 'Upload' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
        ENDIF.
      WHEN mc_cancel_mode.
        _set_toolbar ' ' 'RFSH' icon_refresh 'Refresh' 'Refresh' ' '.
      ...
      WHEN mc_create_mode.
        IF ms_header_config-zcreate EQ abap_true.
          _set_toolbar ' ' 'ADDR' icon_insert_row 'Add Row' 'Add' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
          _set_toolbar ' ' 'DELR' icon_delete_row 'Delete Row' 'Delete' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
          _set_toolbar ' ' 'SAVE' icon_system_save 'Save' 'Save' ' '.
          _set_toolbar '3' '' '' '' '' ' '.
          _set_toolbar ' ' 'CANC' icon_cancel 'Cancel' 'Cancel' ' '.
  ...
  METHOD handle_user_command.
    ...
    TRY.
        PERFORM (lv_form_name) IN PROGRAM (mv_callback_progid) IF FOUND.
        IF e_ucomm EQ 'BAHP'. "HELP GUIDE공통 호출
  ...
  METHOD is_change_mode.
    IF mv_mode EQ mc_change_mode.
      rv_return = abap_true.
    ELSE.
  ...
  METHOD is_create_mode.
    IF mv_mode EQ mc_create_mode.
      rv_return = abap_true.
    ELSE.
      rv_return = abap_false.
    ENDIF.
  ...
  METHOD is_display_mode.
    IF mv_mode EQ mc_display_mode.
      rv_return = abap_true.
    ELSE.
      rv_return = abap_false.
    ENDIF.
  ...
  METHOD set_all_field_changeable.
    CLEAR ms_fieldcat.
    LOOP AT mt_fieldcat INTO ms_fieldcat.
      ms_fieldcat-edit = abap_true.
      MODIFY mt_fieldcat FROM ms_fieldcat INDEX sy-tabix TRANSPORTING edit.
      CLEAR ms_fieldcat.
    ENDLOOP.
  ENDMETHOD.


  METHOD set_all_field_unchangeable.
    CLEAR ms_fieldcat.
    LOOP AT mt_fieldcat INTO ms_fieldcat.
      CLEAR ms_fieldcat-edit.
      MODIFY mt_fieldcat FROM ms_fieldcat INDEX sy-tabix TRANSPORTING edit.
      CLEAR ms_fieldcat.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD set_alv_creatable.
    ms_header_config-zcreate = iv_flag.
  ENDMETHOD.


  METHOD set_alv_deletable.
    ms_header_config-zdelete = iv_flag.
  ENDMETHOD.
  ...
  METHOD set_alv_updatable.
    ms_header_config-zupdate = iv_flag.
  ENDMETHOD.


  METHOD set_alv_uploadable.
    ms_header_config-zexcel = iv_flag.
  ENDMETHOD.
  ...
  METHOD set_celltab.
    FIELD-SYMBOLS : <ls_data_line> TYPE any,
                    ...
                    <lt_outtab>    TYPE STANDARD TABLE.
    ASSIGN mt_outtab->* TO <lt_outtab>.
    IF <lt_outtab> IS ASSIGNED AND ms_layout-stylefname IS NOT INITIAL.
      LOOP AT mt_celltab INTO ms_celltab.
        READ TABLE <lt_outtab> ASSIGNING <ls_data_line> INDEX ms_celltab-rownum.
        IF <ls_data_line> IS ASSIGNED.
          ASSIGN COMPONENT ms_layout-stylefname OF STRUCTURE <ls_data_line> TO <lt_celltab>.
  ...
  METHOD set_cell_changeable.
    DATA : lt_celltab TYPE lvc_t_styl,
           ls_celltab TYPE lvc_s_styl.
    DATA lv_tabix TYPE sy-tabix.

    READ TABLE mt_celltab INTO ms_celltab WITH TABLE KEY rownum = iv_rowid.
    IF sy-subrc EQ 0.
      lv_tabix = sy-tabix.
      lt_celltab = ms_celltab-celltab.
      READ TABLE lt_celltab INTO ls_celltab WITH KEY fieldname = iv_fieldname.
      mv_tabix = sy-tabix.
      IF sy-subrc EQ 0.
        ls_celltab-style = cl_gui_alv_grid=>mc_style_enabled.
        MODIFY lt_celltab FROM ls_celltab INDEX mv_tabix TRANSPORTING style.
      ELSE.
        ls_celltab-fieldname = iv_fieldname.
        ls_celltab-style = cl_gui_alv_grid=>mc_style_enabled.
        INSERT ls_celltab INTO TABLE lt_celltab.
      ENDIF.
      ms_celltab-celltab = lt_celltab.
      MODIFY mt_celltab FROM ms_celltab INDEX lv_tabix.
    ELSE.
      ms_celltab-rownum = iv_rowid.
      ls_celltab-fieldname = iv_fieldname.
      ls_celltab-style = cl_gui_alv_grid=>mc_style_enabled.
      APPEND ls_celltab TO lt_celltab.
      ms_celltab-celltab = lt_celltab.
      INSERT ms_celltab INTO TABLE mt_celltab.
  ...
  METHOD set_cell_color.
    ...
    READ TABLE mt_coltab INTO ms_coltab WITH TABLE KEY rownum = iv_rowid.
    lv_tabix = sy-tabix.
    IF sy-subrc EQ 0.
      lt_coltab = ms_coltab-coltab.
      READ TABLE lt_coltab INTO ls_coltab WITH KEY fname = iv_fieldname.
      mv_tabix = sy-tabix.
      IF sy-subrc EQ 0.
        ls_coltab-color-col = iv_color(1).
        ls_coltab-color-int = iv_color+1(1).
        ls_coltab-color-inv = iv_color+2(1).
        MODIFY lt_coltab FROM ls_coltab INDEX mv_tabix.
      ELSE.
        ls_coltab-fname = iv_fieldname.
        ls_coltab-color-col = iv_color(1).
        ls_coltab-color-int = iv_color+1(1).
        ls_coltab-color-inv = iv_color+2(1).
        INSERT ls_coltab INTO TABLE lt_coltab.
      ENDIF.
      ms_coltab-coltab = lt_coltab.
      MODIFY mt_coltab FROM ms_coltab INDEX lv_tabix.
*      INSERT ms_coltab INTO TABLE mt_coltab.
    ELSE.
      ms_coltab-rownum = iv_rowid.
      ls_coltab-fname = iv_fieldname.
      ls_coltab-color-col = iv_color(1).
      ls_coltab-color-int = iv_color+1(1).
      ls_coltab-color-inv = iv_color+2(1).
  ...
  METHOD set_coltab.
    ...
    IF <lt_outtab> IS ASSIGNED AND ms_layout-ctab_fname IS NOT INITIAL.
      LOOP AT mt_coltab INTO ms_coltab.
        READ TABLE <lt_outtab> ASSIGNING <ls_data_line> INDEX ms_coltab-rownum.
        IF <ls_data_line> IS ASSIGNED.
          ASSIGN COMPONENT ms_layout-ctab_fname OF STRUCTURE <ls_data_line> TO <lt_coltab>.
